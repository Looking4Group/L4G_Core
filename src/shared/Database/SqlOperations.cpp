...
void SqlResultQueue::Update()
{
    ...
    Looking4group::IQueryCallback* callback = NULL;
    while (next(callback))
    {
        callback->Execute();
        delete callback;
...
bool SqlQueryHolder::Execute(Looking4group::IQueryCallback * callback, SqlDelayThread *thread, SqlResultQueue *queue)
{
