        ...
        for (int node = 0; node < BG_AB_DYNAMIC_NODES_COUNT; ++node)
        {
            ...
            if (m_NodeTimers[node])
            {
                ...
                else
                {
                    ...
                    //Quest Support The Battle for Arathi Basin!
                    for (BattleGroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                        if (Player* plr = sObjectMgr.GetPlayer(itr->first))
                        {
                            if (teamIndex == plr->GetTeamId())
                            {
                                switch (node)
                                {
                                    case BG_AB_NODE_STABLES:
                                        plr->KilledMonster(15001, 0);
                                        break;
                                    case BG_AB_NODE_BLACKSMITH:
                                        plr->KilledMonster(15002, 0); 
                                        break;
                                    case BG_AB_NODE_FARM:
                                        plr->KilledMonster(15003, 0); 
                                        break;
                                    case BG_AB_NODE_LUMBER_MILL:
                                        plr->KilledMonster(15004, 0); 
                                        break;
                                    case BG_AB_NODE_GOLD_MINE:
                                        plr->KilledMonster(15005, 0);
                                        break;
                                    default:
                                        ASSERT(0);
                                }
