...
bool TerrainInfo::IsPathFindingEnabled(const Unit* unit) const
{
    const TerrainSpecifics* specifics = GetSpecifics();
    ...
    if (!sWorld.getConfig(CONFIG_MMAP_ENABLED))
            return false;
...
bool TerrainInfo::IsPathfindingForceEnabled(const Unit* unit) const
    {
        if (const Creature* pCreature = dynamic_cast<const Creature*>(unit))
        {
            if (const CreatureInfo* pInfo = pCreature->GetCreatureInfo())
            {
                if (pInfo->flags_extra & CREATURE_EXTRA_FLAG_MMAP_FORCE_ENABLE)
                    return true;
            }
        }
...
bool TerrainInfo::IsPathfindingForceDisabled(const Unit* unit) const
    {
        if (const Creature* pCreature = dynamic_cast<const Creature*>(unit))
        {
            if (const CreatureInfo* pInfo = pCreature->GetCreatureInfo())
            {
                if (pInfo->flags_extra & CREATURE_EXTRA_FLAG_MMAP_FORCE_DISABLE)
                    return true;
            }
        }
        return false;
