...
namespace VMAP
{
    ...
    bool ModelInstance::intersectRay(const G3D::Ray& pRay, float& pMaxDist, bool pStopAtFirstHit, uint32 mapID) const
    {
        if (!iModel)
        {
#ifdef VMAP_DEBUG
        ...
        bool hit = iModel->IntersectRay(modRay, distance, pStopAtFirstHit, mapID);
        if(hit)
        {
            distance *= iScale;
            pMaxDist = distance;
